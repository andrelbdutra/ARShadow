...
function initialize()
{
	...
	fetch('assets/images/contours.txt')
...
//		sourceType: 'image', sourceUrl: 'my-images/index.jpeg',
		sourceType: 'image', sourceUrl: image,
//		sourceWidth: 640,
//		sourceHeight: 480,
//		displayWidth: 640,
//		displayHeight: 640
...
// 	origLight.shadow.mapSize.height = 4096;
	...
	// light2.position.set(-0.04780066504223751, 5.23935357881906, -3.1428021252333442)
...
function setLightFromMask(list, debug = false, inputVector = [])
{
	...
	var groundTruth = new THREE.Vector3(0, 1, 0);
	...
	{
		let x        = parseInt(list[k++]) * m + padw;
		let y        = parseInt(list[k++]) * m + padh;
		mouse.x  =  (x / w) * 2 - 1;
		mouse.y  = -(y / h) * 2 + 1;
...
function enhanceDirectionalLight(mask, mv, groundTruth, initialVector, objectPosition, alpha, origOpacity, debug, vDebug, v3len, first = true)
{
	...
	if (!first)
	{
		...
		if (debug)
		{
			// imprime o grafico dew precisão //////////////////////////////////////////////////////////////////////////////////////////////////////
...
function getRenderValue(v0, v1, mask)
{
	...
	var c10 = 0;
	var c11 = 0;
	var img = ctx.getImageData(pw, ph, 256, 256, {willReadFrequently: true});
...
//	console.log(cw, ch, pw, ph);
	...
	if (w > h)
	{
		img1 = ctx.getImageData(0, 0, pw, 256, {willReadFrequently: true});
		img2 = ctx.getImageData(cw - pw, 0, pw, 256, {willReadFrequently: true});
		var n = pw * 256;
	}
	else
	{
		img1 = ctx.getImageData(0, 0, 256, ph, {willReadFrequently: true});
		img2 = ctx.getImageData(0, ch - ph, 256, ph, {willReadFrequently: true});
		var n = ph * 256;
	}
	for (var j = 0; j < n; j++)
	{
